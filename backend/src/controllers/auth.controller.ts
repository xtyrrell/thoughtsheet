...
const generateToken = (userId: mongoose.Types.ObjectId) => {
  return jwt.sign({ id: userId }, requireEnvVar('SECRET'), { expiresIn: '60d' })
}
...
routes.post('/submit-code', async (req: Request, res: Response) => {
  ...
  if (!user) throw new NotFoundError('A user with this phone number was not found.')
  ...
  if (!valid) {
    return res.send(`oh no, you're code ${otp} wasn't valid; we were expecting ${expectedOtp}.`)
  }
  ...
  return res.send(`You're in! Your JWT encoding your user id ${user._id} is ${token}`)
})

/**
 * Logs in or signs up the user with this email and password.
 * 
...
routes.post('/login', async (req: Request, res: Response) => {
  ...
  if (!user) {
    ...
    const hashedPassword = await bcrypt.hash(password, parseInt(requireEnvVar('HASH_ROUNDS')))
    const user = await User.create({ email, password: hashedPassword })

    // and log them in
    const token = generateToken(user._id)
    const responseText = `
    Created a new account for you! Your JWT encoding your user id is ${user._id} and token is ${token}
  ...
  if (!valid) {
    return res.send(`oh no, your password ${password} wasn't valid; we were expecting ${user.password}.`)
  }
