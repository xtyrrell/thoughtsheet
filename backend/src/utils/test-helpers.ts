  ...
  beforeEach(async () => {
    try {
      await mongoose.connect(
        requireEnvVar("MONGO_URL", "mongodb://localhost"),
        {
    ...
    if (mongoose?.connection?.db?.databaseName !== "test") {
      ...
      throw new Error(
        "Wait stop testing, for some reason we didn't connect to the `test` database! As a safety measure, this is an error."
      );
    }
  });
